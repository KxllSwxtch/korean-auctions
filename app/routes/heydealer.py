...
@router.get("/health")
async def heydealer_health():
    """Health check — проверяет возможность авторизации в HeyDealer API."""
    try:
        cookies, headers = heydealer_auth.get_valid_session()
        if cookies and headers:
            return {"status": "ok", "has_data": True, "authenticated": True}
        return {"status": "error", "has_data": False, "authenticated": False}
    except Exception:
        return {"status": "error", "has_data": False, "authenticated": False}
