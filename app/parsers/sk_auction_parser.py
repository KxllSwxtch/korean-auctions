    ...
    def _parse_single_car_json(self, item: Dict[str, Any]) -> Optional[SKAuctionCar]:
        ...
        try:
            ...
            car = SKAuctionCar(
                ...
                # Auction info
                exhi_no=item.get("exhiNo") or "",
                auction_date=item.get("auctDt") or "",
                lane_div=item.get("laneDiv") or "",
                exhi_div_cd=item.get("exhiDivCd") or "01",
                exhi_stat_cd=item.get("exhiStatCd") or "02",
                exhi_regi_stat_cd=item.get("exhiRegiStatCd") or "02",
                # Vehicle codes
                mdl_cd=item.get("mdlCd") or "",
                car_grp_cd=item.get("carGrpCd") or "",
                # Vehicle names
                car_name=item.get("carCdNm") or "",
                model_name=item.get("mdlCdNm") or "",
                generation_name=item.get("carGrpCdNm") or "",
                # Technical specs
                year=int(item.get("regiYyyy") or 0),
                mileage=item.get("km") or 0,
                transmission=item.get("trnsCdNm") or "",
                transmission_code=item.get("trnsCd") or "01",
                fuel_type=item.get("fuelCdNm") or "",
                color=item.get("colorCdNm") or "",
                color_code=item.get("colorCd") or "",
                # Condition
                accident_grade=item.get("accdScoreCdNm") or item.get("accdScoreCd") or "",
                exterior_grade=item.get("outScoreCdNm") or item.get("outScoreCd") or "",
                # Pricing
                starting_price=starting_price,
                starting_price_won=starting_price_won,
                # Location
                parking_location=item.get("pkltNoNm") or "",
                # Stats
                view_count=item.get("viewCnt") or 0,
                row_number=item.get("no") or 0,
                # Watchlist
                concern_yn=item.get("consYn"),
                concern_search_mode=item.get("concSearchMode") or "none",
                # Status names
                exhi_div_name=item.get("exhiDivCdNm") or "",
                exhi_stat_name=item.get("exhiStatCdNm") or "",
                exhi_regi_stat_name=item.get("exhiRegiStatCdNm") or "",
                # Image
