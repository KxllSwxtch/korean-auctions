...
class EncarService:
    ...
    async def get_catalog(
        ...
        try:
            ...
            # Do not percent-encode Korean, spaces or pipes here: aiohttp/yarl
            # encodes them exactly once when it builds the outgoing request,
            # which is what Encar expects. `+`, `&` and `#` are the
            # exception — yarl leaves them as-is and Encar reads each with
            # the wrong meaning (see app/core/encar_query.py) — so only
            # those three are pre-escaped below.
            ...
            url = f"{self.ENCAR_API_URL}/search/car/list/premium?q={escaped_q}&sr={escaped_sr}&count={count_str}"
