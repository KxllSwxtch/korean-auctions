...
#: Метка на схеме повреждений для каждого типа ремонта.
#:
#: Это символы корейского 성능·상태점검기록부, а не первые буквы английских слов:
#: 교환 — это X, 판금·용접 — W. Раньше здесь стояло "E" от "exchange", и корейский
#: покупатель прочитал всю схему как ошибочную. 도색 на самом бланке метки не имеет
#: (респрей фиксируется через 도막 측정), поэтому для него оставлена "P" — метка
#: аукционных 상태표, её же присылает Autohub.
REPAIR_MARKS: Dict[str, str] = {
    "exchange": "X",
    "weld": "W",
    "painted": "P",
}
...
def repair_mark(repair: Optional[str]) -> str:
    """Метка маркера по типу ремонта.

    Пустая строка для 'none' и для любых значений, которых нет в словаре:
    безымянный маркер честнее выдуманной буквы.
    """
    return REPAIR_MARKS.get(repair or "", "")
