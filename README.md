...
- `AUCTION_PROXY_HOST`, `AUCTION_PROXY_USERNAME`, `AUCTION_PROXY_PASSWORD` —
  ...
  Render/AWS кодом 403 «Request blocked», а с 2026-09-15 — кодом 407 с пустым
  телом; с корейских residential-IP пула те же URL работают. Блокировкой
  считаются 403 и 407 с любым телом, а 429/503 — только со страницей
  CloudFront. Поэтому `/api/catalog`, `/api/nav` и `/api/readside/*` идут
  ...
  CloudFront. Сырой 407 наружу не отдаётся никогда (fetch в Node считает его
  сетевой ошибкой): если пул отклонил собственные учётные данные (407 на
  CONNECT), ответ — `502 proxy_error`, а `last_proxy_status` в диагностике
  показывает 407.
