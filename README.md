...
`q`, `sr` и `inav` пересылаются в Encar как есть, **кроме** `+`, `&` и `#` —
эти три символа percent-кодируются на бэкенде (`app/core/encar_query.py`)
перед сборкой upstream-URL. Корейский текст и `|` кодируются ровно один раз
самим aiohttp/yarl — их percent-кодировать на клиенте не нужно (и нельзя:
двойное кодирование Encar отвечает `Count: 0`).

Вызывающая сторона обязана:
- отправлять `+` как `%2B` (обычный результат `encodeURIComponent`, `+`
  в query-строке Encar читает как пробел);
- экранировать grammar Encar внутри значений самостоятельно — `.` → `_.`,
  `)` → `_)` — до того, как значение попадёт в `q`.
