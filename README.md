...
Локально те же переменные кладутся в `.env`. Полный шаблон — `env.example`:

```bash
cp env.example .env   # затем заполнить значения
```

> `Settings` работает с `extra="forbid"`: ключ в `.env`, не объявленный полем
> в `app/core/config.py`, роняет приложение на импорте. Поэтому используйте
> только имена из `env.example` — новую переменную сначала объявите в
> `Settings`. Правило проверяется `tests/test_env_example.py`.
...
- `AUCTION_PROXY_POOL` — дополнительные записи того же пула, JSON-массив.
  ...
  Пул обходится round-robin: `AsyncHttpClient` берёт следующий прокси на каждый
  запрос, а HappyCar переключается на следующий при совпадении IP или по
  истечении времени жизни сессии — то есть два и более провайдера дают реальный
  failover. Поля записи: `host` (`"host:port"`, без схемы и без учётных данных),
  `username`, `password`; необязательные `name` (метка для логов) и
  `supports_sticky`. Имена всех записей видны при старте в строке
  `🔐 AsyncHttpClient pool: [...]`.

  ```
  AUCTION_PROXY_POOL='[{"name":"provider-kr","host":"host:port","username":"user","password":"pass"}]'
  ```

  В `.env` значение обязательно берите в **одинарные** кавычки — иначе
  python-dotenv не сохранит внутренние двойные. В дашборде Render вставляйте
  чистый JSON без внешних кавычек. Одна переменная вместо нумерованных имён
  (`AUCTION_PROXY_HOST_2` и т. д.) выбрана осознанно: при `extra="forbid"`
  необъявленное имя в `.env` уронило бы приложение на импорте.
...
### Параметры `/api/catalog` и `/api/nav`

Вызывающая сторона кодирует каждый параметр ровно один раз — обычным
`encodeURIComponent` или `params=` своего HTTP-клиента. Starlette декодирует
query-строку, и маршрут получает исходные значения `q`, `sr` и `inav`.

Upstream-URL для Encar бэкенд собирает сам: корейский текст и `|` ровно один
раз percent-кодирует aiohttp/yarl, а `+`, `&` и `#` заранее кодирует
`app/core/encar_query.py`, потому что yarl их не экранирует. URL собирается
вручную, а не через `params=`, потому что при двойном кодировании (`%` → `%25`)
Encar отвечает `Count: 0`.

Вызывающая сторона обязана:
- передавать `+`, `&` и `#` внутри значений как `%2B`, `%26` и `%23` (это
  обычный результат `encodeURIComponent`): сырой `+` в query-строке
  декодируется в пробел, сырой `&` разрезает параметр, а всё после сырого `#`
  до сервера не доходит;
- экранировать grammar Encar внутри значений `q` самостоятельно — `.` → `_.`,
  `)` → `_)` — до кодирования параметра.

Пример: значение `가솔린+전기` (гибридный тип топлива) должно прийти как
`가솔린%2B전기`, иначе Encar видит `가솔린 전기` (пробел вместо `+`) и
возвращает `Count: 0` вместо тысяч подходящих объявлений.

`/api/nav` не принимает `sr` — замер вживую показал прирост тела ответа
всего ~1 КБ при том, что счётчики фасетов от сортировки не зависят.
...
Единственный источник правды по переменным — `env.example`: там перечислены
все поля `Settings` с группировкой и комментариями. Создайте `.env` из него:

```bash
cp env.example .env
```

Минимум, чтобы поднялись все провайдеры: учётные данные из раздела
[🔐 Авторизация](#-авторизация) плюс `GLOVIS_PROXY_*` (Glovis fail-closed) и
`AUCTION_PROXY_*` (обязателен для HappyCar). Что именно не задано, покажет
`GET /healthz/ready` — без раскрытия значений.

## 🐛 Troubleshooting

### 503 `AUTH_MISCONFIGURED` на `/api/v1/<провайдер>/*`

Учётные данные провайдера не заданы в окружении — это не сбой аукциона, и
повторный запрос не поможет (поэтому ответ приходит без `Retry-After`).

```bash
curl -s localhost:8000/healthz/ready | python -m json.tool
```

Ответ перечислит по каждому сервису, каких имён переменных не хватает
(только имена, без значений). Те же строки пишутся в лог при старте.
Добавьте недостающие переменные в `.env` (шаблон — `env.example`) и
перезапустите: `Settings` кэшируется на импорте, поэтому правка `.env`
без рестарта ничего не изменит.

### 503 `AUTH_UNAVAILABLE` на `/api/v1/<провайдер>/*`

Учётные данные заданы, но вход не удался — это временное состояние, ответ
приходит с `Retry-After`. Проверьте доступность сайта, логи на предмет SSL
ошибок и не заблокирован ли аккаунт (после серии неудач включается
15-минутный lockout).

### Приложение падает на старте с `ValidationError`

В `.env` есть ключ, не объявленный полем в `app/core/config.py` — `Settings`
работает с `extra="forbid"`. Имя ключа будет в тексте ошибки: либо уберите
его, либо объявите поле в `Settings`. Переменные реального окружения этим
правилом не затронуты, оно касается только файла `.env`.
