...
BASE_URL = "https://korean-auctions.onrender.com"
...
def get_models(manufacturer_code: str):
    ...
    try:
        response = requests.get(f"{BASE_URL}/api/v1/ssancar/models/{manufacturer_code}", timeout=10)
        data = response.json()
        if data.get('success'):
            return data['models']
    except Exception as e:
...
def test_model_code(manufacturer: str, code: int, week_no: str = "2") -> Tuple[bool, int]:
    """
    ...
    try:
        ...
        response = requests.post(
            f"{BASE_URL}/api/v1/ssancar/search",
            json=payload,
            timeout=10
        )
...
def find_model_code_range(manufacturer: str, start: int = 600, end: int = 800) -> Dict[int, str]:
    """
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                if success:
                    ...
                    logger.info(f"✅ Found: {manufacturer} code {code} -> {model_name} ({count} cars)")
            except Exception as e:
                logger.error(f"Error testing code {code}: {e}")
    
    return working_codes

def discover_model_codes_for_manufacturer(manufacturer_code: str, manufacturer_name: str) -> Dict:
    """
    ...
    for api_model in api_models:
        ...
        for code, model_name in all_working_codes.items():
            ...
            if (api_name.lower() in model_name.lower() or 
                model_name.lower() in api_name.lower() or
                api_model['name'] in model_name):
                matched_code = code
                break
        ...
        if matched_code:
            ...
            logger.info(f"  Mapped: {api_name} - API:{api_code} -> Actual:{matched_code}")
        else:
...
""".format(date=time.strftime('%Y-%m-%d %H:%M:%S'))
    ...
    for manufacturer, data in mappings.items():
        ...
        ts_manufacturer = manufacturer.replace('벤츠', 'Mercedes').replace('기아', 'Kia').replace('현대', 'Hyundai')
...
def main():
    ...
    logger.info(f"BMW code 692 (3 Series): {'✅ Working' if test_success else '❌ Failed'} ({count} cars)")
    
    test_success, count, _ = test_model_code("BMW", 694)  # BMW 5 Series
    logger.info(f"BMW code 694 (5 Series): {'✅ Working' if test_success else '❌ Failed'} ({count} cars)")
    
    test_success, count, _ = test_model_code("벤츠", 655)  # Mercedes C-Class
    logger.info(f"Mercedes code 655 (C-Class): {'✅ Working' if test_success else '❌ Failed'} ({count} cars)")
    ...
    for manufacturer, data in all_mappings.items():
        logger.info(f"{manufacturer}: {data['mapped_count']}/{data['api_models_count']} models mapped")
